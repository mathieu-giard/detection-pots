...
public class Selection {
	...
	static public Pt[][] selec(BufferedImage img, double seuilTinf, double seuilTsup,
		...
		Pt retour[][] = new Pt[img.getWidth()/SIZEFACTOR][img.getHeight()/SIZEFACTOR];

		
		for (int x = 0; x < img.getWidth()/SIZEFACTOR; x++) {
			for (int y = 0; y < img.getHeight()/SIZEFACTOR; y++) {// parcourir l'image
				retour[x][y] = new Pt(x, y, false);
			
				Color c = new Color(img.getRGB(x*SIZEFACTOR, y*SIZEFACTOR));// prendre des valeurs RGB
														// de chaque pixel
				int r = c.getRed();
				int g = c.getGreen();
				int b = c.getBlue();
